...
# Delete the variable group used by $VG_NAME
delete_vg() {
    local vg_id=$1
    az pipelines variable-group delete --organization "${ORG}" --project "${PROJECT}" --id "${vg_id}" --yes --only-show-errors
}
...
populate_vg(){
    ...
    for vg in "${!variable_groups[@]}"; do
        az pipelines variable-group variable create --organization "${ORG}" --project "${PROJECT}" --id "$vg_id" --name "${vg}" --value "${variable_groups[$vg]}" --only-show-errors
    done
}
